...
#define diffmilli(start, end) (_diffsec(start, end)*1e3 + _diffnsec(start, end)/1e6)

#define diffmicro(start, end) (_diffsec(start, end)*1e6 + _diffnsec(start, end)/1e3)

#define diffnano(start, end) (_diffsec(start, end)*1e9 + _diffnsec(start, end))
...
struct timeval tv;
static char* randstring(uint max_length) {
...
static inline void free_strings(dval_t *strings, size_t n) {
...
void test_dict_initialized(ssize_t maxlen) {
        ...
        dict_free(mp);
        Fail:
        free(keys); free_strings(values, maxlen);
}
