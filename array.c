...
static inline int
array_resize_helper (entry_list *arr, ssize_t new_size)
{
...
static int
array_setitem (entry_list *arr, ssize_t ix, dt_entry *en)
{
  checkindex (arr, ix);
  arr->ar_items[ix] = en;
  return 1;
...
dt_entry *
array_getitem (entry_list *arr, ssize_t ix)
{
  checkindex (arr, ix);
...
array_resize (entry_list *arr)
{
  ...
  else if (arr->ar_used_count > MINSIZE
           && arr->ar_used_count < (arr->ar_allocated_count >> 1))
    {
...
int
array_grow (entry_list *arr, ssize_t n)
{
...
ssize_t
array_lookup (entry_list *arr, dt_entry *en)
{
  if (!arr)
    return -1;
...
int
array_append (entry_list *arr, dt_entry *item)
{
  assert (arr);
...
static inline void
checkindex (entry_list *arr, size_t index)
{
  if ((size_t)index > (size_t)arr->ar_used_count)
    {
      fprintf (stderr, "index %zd out of bounds for ar_items of size %zd",
               arr->ar_used_count, index);
      exit (EXIT_FAILURE);
    }
}

void
array_insert (entry_list *arr, ssize_t index, dt_entry *item)
{
  assert (arr);
...
int
array_extend (entry_list *arr, dt_entry **ens, ssize_t nd)
{
  if (!arr || !ens)
    return -1;
  if (nd == 0)
...
void
array_delete (entry_list *arr, ssize_t index)
{
  assert (arr);
...
void
array_free_items (entry_list *arr)
{
  dt_entry *entry = arr->ar_items[0];
  if (arr->ar_isfirst == 1)
    {
...
array_copy (entry_list *arr)
{
  ...
  arr_copy->ar_items
      = SAFEMALLOC (sizeof (dt_entry *) * arr->ar_allocated_count);
