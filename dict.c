...
#define DT_SET_VALUE(dt, ix, value) (array_getitem(&dt->dt_entries, ix)->et_value = value)
...
#define DT_SET_ENTRY(dt, ix, entry) (array_setitem(&(dt->dt_entries), ix, entry))
...
#define ACTUAL_SIZE(size) (IS_POWER_OF_2(size) ? size: (1 << (64 - __builtin_clzl(size))))
...
static inline void assert_consistent(dict *dt)
{
...
dict_new_initialized(dkey_t *keys, dval_t *values, size_t n)
{
        ...
        if (n == 0) {
                ...
                for (ssize_t i = 0; i < n; i++) {
                        ...
                        if (dict_insert_with_hash(d, hash(keys[i]),
                                                  &keys[i], val) == -1) {
                                return NULL;
                        }
                }
...
dict_new_index(dict *dt, ssize_t minsize)
{
        ...
        if (s <= 0xff) { // 255 | (2^8) - 1
                ...
                es = 1;
        } else if (s <= 0xffff) {  // 65535 | (2^16) - 1
                (dt->dt_indices) = SAFEMALLOC(sizeof(int16_t) * s);
                es = 2;
        }
#if SIZEOF_VOID_P > 4
        else if (s > 0xffffffff) {  // (2^32) - 1
...
#if SIZEOF_VOID_P > 4
        else if (s > 0xffffffff) {  // 4294967295 | (2^32) - 1
...
int
dict_insert_with_hash(dict *dt, hash_t hash, const dkey_t *key, const dval_t *value)
{
        if (!value || !key || !dt)
                return -1;
        dval_t oldvalue;
...
void dict_printkeys(keyobj *keyobj)
{
        if (!keyobj) {
                fprintf(stderr, "Null pointer\n");
...
itemobj *dict_getitems(dict *dt)
{
        ...
        for (ssize_t i = 0, j = 0; i < dt->dt_entries.ar_used; i++) {
                if (entries[i] != NULL) {
                        t = (item) {.key=entries[i]->et_key, .value=entries[i]->et_value};
                        items[j++] = t;
                }
        }
        it->items = items;
...
static void repr_item(item *t, FILE *stream)
{
        fprintf(stream, "(");
...
dict_repr(dict *dt, FILE *stream)
{
        ...
        if (dt->dt_nentries == 0) {
                ...
                for (ssize_t i = 0; i < dt->dt_entries.ar_used; entry = dt->dt_entries.ar_items[++i]) {
                        if (entry) {
                                if (first)
                                        first = false;
                                else
...
static int val_eq(dval_t *self, dval_t *other)
{
        return strcmp(*self, *other) == 0;
}

static int key_eq(const dkey_t *self, const dkey_t *other)
{
        return *self == *other;
}
...
dict_update(dict *a, dict *b, int override)
{
        ...
        if (array_grow(&a->dt_entries, a->dt_entries.ar_used + b->dt_nentries) == -1) {
...
ssize_t dict_sizeof(dict *dt)
{
        ...
        t += (sizeof(dt_entry) * dt->dt_nentries) + (sizeof(dt_entry *) * dt->dt_entries.ar_allocated);
        t += sizeof(dict);
        ...
        int es;
        if (s <= 0xff) es = 1;
        else if (s <= 0xffff) es = 2;
#if SIZEOF_VOID_P > 4
        else if (s > 0xffffffff) es = 8;
#endif
        else es = 4;
